...
void checkStringDictColumnData(ArrowStorage& storage,
  ...
  auto& dict =
      *storage.getDictMetadata(col_info->type.get_comp_param())->stringDict;
...
TEST_F(ArrowStorageTest, CreateTable_SharedDict) {
  ...
  auto tinfo = storage.createTable("table1",
                                   {{"col1", type_dict0},
                                    ...
                                    {"col5", type_dict2}});
  auto col_infos = storage.listColumns(*tinfo);
  CHECK_EQ(col_infos[0]->type.get_comp_param() & 0xffffff, 1);
  CHECK_EQ(col_infos[1]->type.get_comp_param() & 0xffffff, 2);
  CHECK_EQ(col_infos[2]->type.get_comp_param() & 0xffffff, 3);
  CHECK_EQ(col_infos[3]->type.get_comp_param() & 0xffffff, 2);
  CHECK_EQ(col_infos[4]->type.get_comp_param() & 0xffffff, 3);
  CHECK(storage.getDictMetadata((TEST_SCHEMA_ID << 24) + 1));
  CHECK(storage.getDictMetadata((TEST_SCHEMA_ID << 24) + 2));
  CHECK(storage.getDictMetadata((TEST_SCHEMA_ID << 24) + 3));
}
...
void Test_ImportCsv_Dict(bool shared_dict,
  ...
  if (shared_dict) {
    ...
    auto dict1 =
        storage.getDictMetadata(col1_info->type.get_comp_param())->stringDict;
    ...
    auto dict2 =
        storage.getDictMetadata(col2_info->type.get_comp_param())->stringDict;
